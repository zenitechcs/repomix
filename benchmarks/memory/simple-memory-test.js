...
async function runTest() {
  ...
  console.log(`📊 Initial: Heap ${initialMemory.heap}MB, RSS ${initialMemory.rss}MB`);
  ...
  for (let i = 1; i <= ITERATIONS; i++) {
    try {
      ...
      if (i % 5 === 0) {
        ...
        console.log(`✅ Iteration ${i}: Heap ${current.heap}MB (+${heapGrowth}%), RSS ${current.rss}MB (+${rssGrowth}%)`);
  ...
  if (!CONTINUOUS) {
    ...
    const heapGrowth = ((finalMemory.heap - initialMemory.heap) / initialMemory.heap * 100).toFixed(1);
    const rssGrowth = ((finalMemory.rss - initialMemory.rss) / initialMemory.rss * 100).toFixed(1);

    console.log(`\n📊 Final Results:`);
    console.log(`   Initial: Heap ${initialMemory.heap}MB, RSS ${initialMemory.rss}MB`);
    console.log(`   Final:   Heap ${finalMemory.heap}MB, RSS ${finalMemory.rss}MB`);
